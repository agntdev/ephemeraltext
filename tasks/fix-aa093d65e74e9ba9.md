# fix-aa093d65e74e9ba9 — Per-message data key not wiped after sealing

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 EPTX

In `bot.ts:244-248`, `sealDraft()` generates a per-message data key via `generateDataKey()`, uses it for encryption and KMS wrapping, but never calls `wipe(dataKey)` to overwrite the key buffer. The `wipe()` function exists in `crypto.ts` and is correctly used in the retrieval path (`retrieval.ts:42-44`), but is absent from the upload/encrypt path. The un-wiped key buffer remains in memory until garbage collection, unnecessarily extending the window where key material can be recovered from memory.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-aa093d65e74e9ba9.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-aa093d65e74e9ba9.json` (and any new command `tests/commands/fix-aa093d65e74e9ba9.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-aa093d65e74e9ba9.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-aa093d65e74e9ba9.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
