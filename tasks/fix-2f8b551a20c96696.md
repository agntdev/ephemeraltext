# fix-2f8b551a20c96696 — Redis TTL lost on time-limited message re-save after read

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 EPTX

In `retrieval.ts:54-55`, after incrementing `readCount` on a time-limited message, `store.save(message)` is called **without** a TTL. In `RedisMessageStore.save()` (`messages.ts:72-79`), calling `set` without `EX` removes any existing Redis key expiry. This means a time-limited message that has been read once will **persist indefinitely in Redis** — its `expiresAt` check still runs on subsequent reads, but if nobody reads it again, the key never auto-expires. The original TTL from `sealDraft()` (`bot.ts:259`) is silently dropped.

**Fix**: compute remaining TTL (`max(1000, expiresAt - now)`) and pass it to `save()` on the re-save path.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-2f8b551a20c96696.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-2f8b551a20c96696.json` (and any new command `tests/commands/fix-2f8b551a20c96696.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-2f8b551a20c96696.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-2f8b551a20c96696.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
