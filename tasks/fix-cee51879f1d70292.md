# fix-cee51879f1d70292 — Multiple independent Redis client instances created

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 EPTX

Three separate subsystems (`createMessageStore`, `createRateLimitBackend`, `createAuditLog`) each create their own `new Redis(url)` client when `REDIS_URL` is set. This wastes Redis connections (3x) and risks hitting connection limits in production. A single shared Redis client should be injected into these backends instead of each creating its own.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-cee51879f1d70292.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-cee51879f1d70292.json` (and any new command `tests/commands/fix-cee51879f1d70292.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-cee51879f1d70292.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-cee51879f1d70292.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
