# fix-b96a059e99e2888c — Multiple Redis connections without lifecycle management

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 EPTX

`createAuditLog()` (`src/audit.ts:55-56`), `createMessageStore()` (`src/messages.ts:134-138`), `createRateLimitBackend()` (`src/ratelimit.ts:54-59`), and the toolkit's session storage (`src/toolkit/session/redis.ts:83`) each create their own `new Redis(url)` client. There is no shared connection pool and no graceful shutdown (`quit`/`disconnect` on SIGTERM). This wastes connections and risks state issues during deployment restarts.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-b96a059e99e2888c.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-b96a059e99e2888c.json` (and any new command `tests/commands/fix-b96a059e99e2888c.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-b96a059e99e2888c.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-b96a059e99e2888c.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
